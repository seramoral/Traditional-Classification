 ...
 *
 *
 *    Variation of RF using IIG criterion (uses RandomTreeRandomCredalSminmax)
 */

/*
 *    RandomForestRandomCredalSminmax.java
 ...
 */
public class RandomForestRandomCredalSminmax 
  extends Classifier 
  ...
  */
  ...
  /**
   * Number of values (NOMaximum valueNO) for s 
  */
  private int m_maxS = 6;
  ...
  public String numFeaturesTipText() {
    return "The number of attributes to be used in random selection (see RandomTreeRandomCredalSminmax).";
  }
   ...
   * displaying in the explorer/experimenter gui
   */
  public String SvalueTipText() {
    return "The paramter s to be used for Credal sets.";
  }

  
  /**
   * Get the valor para s.
   *
   * @return Value of s.
   */
   ...
   * displaying in the explorer/experimenter gui
   */
  public String minSTipText() {
    return "The minimum value for the paramter s to be used for random Credal sets.";
  }
    
  
  /**
   * Get the valor para min_S.
   *
   * @return Value of s.
   */
  public int getminS() {
...
/**
   ...
   * displaying in the explorer/experimenter gui
   */
  public String maxSTipText() {
    return "The number of values (NOmaximum valueNO) for the paramter s to be used for random Credal sets.";
  }
   
  
  /**
   * Get the valor para max_S.
   *
   * @return Value of s.
   */
  public int getmaxS() {
  ...
  public double getMeasure(String additionalMeasureName) {
    ...
    if (additionalMeasureName.equalsIgnoreCase("measureOutOfBagError")) {
      return measureOutOfBagError();
					     ...
					     + " not supported (RandomForestRandomCredalSminmax)");
    }
  }
  ...
  public Enumeration listOptions() {
    ...
    newVector.
	addElement(new Option("\tSet the value for s parameter.\n" +
			      "\t(default 1.0)",
			      "Value", 1, "-Svalue <Value for s>"));
   
    newVector.
	addElement(new Option("\tSet the value minimum for s parameter.\n" +
			      "\t(default 0)",
			      "V", 1, "-V <Value minimum for s>"));    
    newVector.
	addElement(new Option("\tSet the number of values (NOvalue maximumNO) for s parameter.\n" +
			      "\t(default 6)",
			      "N", 1, "-N <Number of values (NOValue maximumNO) for s>"));
  ...
  public void setOptions(String[] options) throws Exception{
    ...
    String valorsString = Utils.getOption("Svalue", options);
    if (valorsString.length() != 0) {
	m_SValue = (new Float(valorsString)).floatValue();
	if ((m_SValue < 0)){ //|| (m_SValue > 1)) {
	  throw new Exception("SValue has to be >= zero !");
	}
    } else {
      m_SValue = 1.0f;
    }

    valorsString = Utils.getOption("V", options);
    if (valorsString.length() != 0) {
	m_minS = Integer.parseInt(valorsString);
	if ((m_minS < 0)){ //|| (m_SValue > 1)) {
	  throw new Exception("Minimum value for s has to be >= zero !");
	}
    } else {
      m_minS = 0;
    }
    
    valorsString = Utils.getOption("N", options);
    if (valorsString.length() != 0) {
	m_maxS = Integer.parseInt(valorsString);
	if ((m_maxS < 0)){ //|| (m_SValue > 1)) {
	  throw new Exception("Number of values for s has to be >= zero !");
	}
    } else {
      m_maxS = 6;
    }
    ...
    /*
    tmpStr = Utils.getOption("VmaxS", options);
    if (tmpStr.length() != 0) {
      m_maxS = Integer.parseInt(tmpStr);
    } else {
      m_maxS = 4;
    }
    */
  ...
  public Capabilities getCapabilities() {
    return new RandomTreeRandomCredalSminmax().getCapabilities();
  }
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    RandomTreeRandomCredalSminmax rTree = new RandomTreeRandomCredalSminmax();
    ...
    rTree.setSvalue (m_SValue);
    rTree.setmaxS (m_maxS);
    rTree.setminS (m_minS);
  ...
  public static void main(String[] argv) {
    runClassifier(new RandomForestRandomCredalSminmax(), argv);
  }
}
